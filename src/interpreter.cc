	...
	if (right == Value::null) {
		...
		_error(
			_T("La asignación no se puede hacer porque los "
			   ...
			   left.type_name().c_str(), right.type_name().c_str()));
	}
...
void Interpreter::CheckResult(Binding& fn, const Function *func_type) {
	if (_ret == Value::null && !func_type->is_void()) {
		...
		_error(_T("La función '%s' debería devolver un '%s'", name.c_str(),
	...
	switch (ast->Type()) {
		...
		case AstType::Identifier: {
			...
			if (namespc_or_class != 0) {
				...
				if (namespc != 0) {
					...
					_error(_T("No se ha encontrado '%s' en el namespace '%s'.", X->name.c_str(),
		...
		case AstType::BinaryExpr: {
			...
			if (X->op == "+=" || X->op == "-=" || X->op == "*=" || X->op == "/=" || X->op == "&=" ||
				...
				switch (X->op[0]) {
					case '&':
						ret = EvalBitop<_And>(left, right);
						break;
					case '|':
						ret = EvalBitop<_Or>(left, right);
						break;
					case '^':
						ret = EvalBitop<_Xor>(left, right);
						break;
				}
				if (ret) {
					return;
					...
					switch (X->op[0]) {
						case '+': {
							if (left.is<Char>() and right.is<Int>()) {
								...
								ret = EvalSumProd<_Add>(left, right);
								break;
							}
						}
						case '*':
							ret = EvalSumProd<_Mul>(left, right);
							break;
						case '-':
							ret = EvalSumProd<_Sub>(left, right);
							break;
						case '/':
							ret = EvalSumProd<_Div>(left, right);
							break;
					}
				} else {
					_curr = left;
		...
		case AstType::IncrExpr: {
			...
			if (after.is<Int>()) {
				...
				if (!CallOperator(op)) {
					_error(_T("El tipo '%s' no tiene 'operator%s'", _curr.type()->TypeStr().c_str(),
							  op.c_str()));
				}
			}

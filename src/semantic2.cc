...
struct SemanticAnalyzer2 : public WithEnvironment
{
...
void SemanticAnalyzer2::visit_binaryexpr_assignment(BinaryExpr* X, 
...
void SemanticAnalyzer2::visit_binaryexpr_op_assignment(char op, Value left, Value right) {
   if (!left.is<Reference>()) {
...
void SemanticAnalyzer2::visit_callexpr_getfunc(CallExpr *X) {
   _curr_node = X;
   Analyze(X->func);
...
bool SemanticAnalyzer2::visit_type_conversion(CallExpr *X, const vector<Value>& args) {
   _curr_node = X;
   _curr_varname = "";
...
void SemanticAnalyzer2::check_arguments(const Function *func_type,
...
bool SemanticAnalyzer2::bind_field(Value obj, string method_name) {
...
bool SemanticAnalyzer2::call_operator(string op, const vector<Value>& args) {
...
void SemanticAnalyzer2::check_condition(Expr *cond, string who) {
   Analyze(cond);
   _curr = Reference::deref(_curr);
   if (!_curr.is<Bool>()) {
...
void SemanticAnalyzer2::check_unknown(Value v, Ast *X, string varname) {
   if (v.is_unknown()) {
...
void SemanticAnalyzer2::eval_arguments(const vector<Expr *>& args,
...
template<class Op>
bool SemanticAnalyzer2::visit_op_assignment(Value left, Value _right) {
...
template<class Op>
bool SemanticAnalyzer2::visit_bitop_assignment(Value left, Value _right) {
   Value right = left.type()->convert(_right);
   if (left.is<Int>() and right.is<Int>()) {
...
template<class Op>
bool SemanticAnalyzer2::visit_sumprod(Value left, Value _right, string what) {
   Value right = left.type()->convert(_right);
   if (right.is_null()) {
...
template<class Op>
bool SemanticAnalyzer2::visit_bitop(Value left, Value right) {
   if (left.is<Int>() and right.is<Int>()) {
...
template<class Op>
bool SemanticAnalyzer2::visit_comparison(Value left, Value right) {
   if (left.is<Int>() and right.is<Int>()) {
      _curr = Value(Op::eval(left.as<Int>(), right.as<Int>()));
      return true;
   }
...
void SemanticAnalyzer2::Analyze(Ast *ast) {
   switch (ast->type()) {
   ...
   case AstType::Literal: {
      ...
      default:
         X->add_error(_T("SemanticAnalyzer2::visit_literal: UNIMPLEMENTED"));
      }
      break;      
   }
   case AstType::Block: {

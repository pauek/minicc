...
Value::Value(int x) : _const(false) {
	_attach(new Box(Int::self, Int::self->alloc(x)));
}

Value::Value(char x) : _const(false) {
	_attach(new Box(Char::self, Char::self->alloc(x)));
}

Value::Value(bool x) : _const(false) {
	_attach(new Box(Bool::self, Bool::self->alloc(x)));
}

Value::Value(float x) : _const(false) {
	_attach(new Box(Float::self, Float::self->alloc(x)));
}

Value::Value(double x) : _const(false) {
	_attach(new Box(Double::self, Double::self->alloc(x)));
}

Value::Value(string x) : _const(false) {
	_attach(new Box(String::self, String::self->alloc(x)));
}

Value::Value(ostream& o) : _const(false) {
	_attach(new Box(OStream::self, &o));
}

Value::Value(istream& i) : _const(false) {
	_attach(new Box(IStream::self, &i));
}

Value::Value(const char *x) : _const(false) {
	_attach(new Box(String::self, String::self->alloc(string(x))));
}

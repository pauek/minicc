...
Parser::Parser(istream *i, std::ostream *err) : _lexer(i), _err(err) {
	static const char *basic_types[] = {"int",	 "char", "string", "double", "float",
										"short", "long", "bool",   "void",	 "vector",
										"list",	 "map",	 "set",	   "pair"};
...
Expr *Parser::parse_exprlist(Ast *parent) {
	...
	do {
		...
		elist->exprs.push_back(_lexer.curr() == '{' ? parse_exprlist(parent)
...
DeclStmt *Parser::parse_declstmt(Ast *parent, bool is_typedef) {
	...
	while (true) {
		...
		if (_lexer.curr() == '=') {
			_lexer.next();
			_skip(stmt);
			item.init = (_lexer.curr() == '{' ? parse_exprlist(item.decl)
